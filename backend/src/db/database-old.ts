...
export async function runQuery(sql: string, params: any[] = []): Promise<any> {
  if (USE_POSTGRES) {
    ...
    // Add RETURNING * clause for INSERT statements to get the inserted row
    ...
    try {
      ...
      // For INSERT with RETURNING, return the first row
      if (pgSql.includes('RETURNING')) {
        return { id: result.rows[0]?.id, row: result.rows[0], changes: result.rowCount };
      }
